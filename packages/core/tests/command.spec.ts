import { App, Command, Logger, Next } from '@spakjs/core'
...
describe('Command API', () => {
  ...
  describe('Execute Commands', () => {
    ...
    it('throw 1 (error in action)', async () => {
      command.config.handleError = () => 'Oops!'
      command.action(() => {
        throw new Error('message 1')
      })

      await expect(command.execute({ session }, next)).eventually.to.equal('Oops!')
    ...
    it('throw 2 (error in next callback)', async () => {
      ...
      await expect(command.execute({ session }, next)).eventually.to.equal('An unknown error has occurred.')
      expect(print.mock.calls).to.have.length(1)
