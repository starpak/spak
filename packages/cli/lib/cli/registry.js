...
function generateCommandHelp(rootName, declarations) {
...
function generateHelp(rootName, rootDecl, children) {
...
function registerDeclarations(cli, declarations) {
    // Collect root commands (each top-level command has one entry)
    const rootCmdNames = new Set();
    ...
    for (const rootName of rootCmdNames) {
        ...
        cmd.action((...args) => {
            ...
            const positional = args.slice(0, -1).filter((a) => typeof a === 'string');
            ...
            for (const decl of declarations) {
                ...
                if (parts[0] !== rootName)
                    continue;
                if (parts.length === 1)
                    continue; // Root command, fallback later
                // Check if it matches the subcommand path
                const subParts = parts.slice(1);
                let match = true;
                for (let i = 0; i < subParts.length; i++) {
                    // If argument placeholder (<xxx> or [xxx]), match any value
                    if (subParts[i].startsWith('<') || subParts[i].startsWith('['))
                        continue;
                    if (subParts[i] !== subArgs[i]) {
                        match = false;
                        break;
                    }
                }
