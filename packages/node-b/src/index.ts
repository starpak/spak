...
export async function fingerprint(root: string, entries: string[]): Promise<string> {
...
export async function readWhitelist(path: string): Promise<Record<string, string>> {
  try {
    await access(path)
...
export async function writeWhitelist(path: string, map: Record<string, string>): Promise<void> {
