...
class Watcher {
  ...
  start() {
    ...
    const triggerLocalReload = this.ctx.debounce(() => this.triggerLocalReload(), this.config.debounce ?? 100)
  ...
  private triggerLocalReload() {
    ...
    for (const filename of this.accepted) {
      backup[filename] = coreRequire.cache[filename] as NodeJS.Module
