...
function escape(source, whitespace = true) {
    if (source === null || source === undefined)
        return '';
    if (Array.isArray(source))
        return source.map(child => escape(child, whitespace)).join('');
    ...
    if (source instanceof Fragment || source instanceof h)
...
function attrEscape(v) {
    return v.replace(ATTR_ESCAPE_RE, c => c === '&' ? '&amp;' : c === '<' ? '&lt;' : c === '>' ? '&gt;' : '&quot;');
}
/**
...
class h {
    ...
    static parse(source, params) {
        ...
        function consumeEntity(innerBuf) {
            // pos is at '&'
            if (source.startsWith('amp;', pos + 1)) {
                pos += 5;
                return '&';
            }
            if (source.startsWith('lt;', pos + 1)) {
                pos += 4;
                return '<';
            }
            if (source.startsWith('gt;', pos + 1)) {
                pos += 4;
                return '>';
            }
            if (source.startsWith('quot;', pos + 1)) {
                pos += 6;
                return '"';
            }
            if (source.startsWith('#39;', pos + 1)) {
                pos += 5;
                return "'";
            }
            // numeric entity
        ...
        function parseTag() {
            ...
            if (recursionDepth >= MAX_RECURSION_DEPTH) {
                ...
                pos = len;
                return rest; // caller will detect non-h/false/null and handle
            }
            recursionDepth += 1;
            try {
                // pos at '<'
        ...
        while (pos < len) {
            ...
            if (c === '<') {
                ...
                const tag = parseTag();
                if (tag instanceof h)
                    rootChildren.push(tag);
                else if (typeof tag === 'string')
                    rootChildren.push(tag); // depth-guard escape
            }
            else if (c === '&') {
